...
- `spotify` is scaffolded with OAuth + PKCE through `chrome.identity.launchWebAuthFlow`. It stores tokens in `chrome.storage.local`, never asks for a password, and does not require a client secret in the extension.
